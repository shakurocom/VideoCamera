...
class ExampleVideoCameraViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if isNewCameraShown {
            let model = CameraModel(options: CameraModel.Options(isAudioAllowed: false, captureModes: [], isVideoFeedEnabled: true))
            cameraModel = model
...
extension ExampleVideoCameraViewController: VideoCameraDelegate {
    ...
    nonisolated func videoCamera(_ videoCamera: VideoCamera, authorizationStatusChanged newValue: AVAuthorizationStatus) {
        DispatchQueue.main.async(execute: {
            ...
            @unknown default:
                fatalError("unknown AVAuthorizationStatus: \(newValue)")
            }
        })
    }
...
extension ExampleVideoCameraViewController: ExampleViewControllerProtocol {
    ...
    static func instantiate(example: Example) -> UIViewController {
        ...
        let exampleVC: ExampleVideoCameraViewController = storyboard.instantiateViewController(withIdentifier: "kExampleVideoCameraViewControllerID")
